...
- (instancetype)initWithCollection:(id<RLMFastEnumerable>)collection objectSchema:(RLMObjectInfo *)objectSchema {
    self = [super init];
    if (self) {
...
- (NSUInteger)countByEnumeratingWithState:(NSFastEnumerationState *)state
    ...
    for (NSUInteger index = state->state; index < count && batchCount < len; ++index) {
        RLMObject *accessor = [[accessorClass alloc] initWithRealm:_realm schema:_info];
        if (_collection) {
